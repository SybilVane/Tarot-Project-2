...
router.get('/reading', (req, res, next) => {
  ...
  .then(cards => {
    ...
    History
    .create({cards_id:[card1._id, card2._id, card3._id], user_id:req.session.currentUser._id})
    .then(() => res.render('cards/cards-reading', {card1, card2, card3, positionA, positionB, positionC}))
    .catch(err => console.error(err))
    })
  .catch(err => console.error(err));
});
